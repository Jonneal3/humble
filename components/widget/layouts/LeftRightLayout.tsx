...
export function LeftRightLayout({
  ...
  const responsivePromptWidth = isMobile 
    ? Math.min(leftColumnWidth, 35) // Cap at 35% for mobile
    : containerWidth < 900 
      ? Math.min(leftColumnWidth, 38) // Cap at 38% for small containers
      : leftColumnWidth; // Use configured width for larger containers

  return (
    <div 
      className="h-full w-full flex flex-col"
      style={{ 
        padding: `${Math.max(4, Math.min(24, containerWidth * 0.015))}px ${Math.max(8, Math.min(24, containerWidth * 0.02))}px`
      }}
    >
      <div className="h-full flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {suggestions.slice(0, 4).map((suggestion, index) => (
                        <button
                          ...
                          disabled={isLoading}
                          className="flex items-center gap-1 border font-medium flex-shrink-0 max-w-[calc(50%-0.375rem)]"
                          style={{
                            ...
                            padding: `${Math.max(4, Math.min(12, containerWidth * 0.008))}px ${Math.max(6, Math.min(16, containerWidth * 0.012))}px`
                          }}
                        >
        ...
        {!isMobile && (
          <div 
            className="flex-1 flex flex-row min-h-0"
            style={{ gap: `${Math.max(8, Math.min(24, containerWidth * 0.012))}px` }}
          >
                ...
                <div className="flex-1 min-h-0 flex flex-col overflow-auto">
                  <div className="flex-1 rounded-2xl p-4 border transition-all duration-300"
                    style={{
                      ...
                      borderWidth: `${config.prompt_border_width || 1}px`,
                      borderStyle: config.prompt_border_style || 'solid',
                      minHeight: `${Math.max(120, Math.min(200, containerWidth * 0.15))}px`
                    }}
                  >
                    <div className="h-full flex flex-col">
                      {/* Upload section */}
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {suggestions.slice(0, config.suggestions_count || 6).map((suggestion, index) => (
                              <button
                                ...
                                onClick={() => onSuggestionClick(suggestion)}
                                disabled={isLoading}
                                className="flex items-center gap-1.5 rounded-lg border group transition-all duration-200 shadow-sm font-medium flex-shrink-0 max-w-[calc(50%-0.25rem)]"
                                style={{
                                  backgroundColor: config.suggestion_background_color || '#ffffff',
                                  borderColor: config.suggestion_border_color || '#e5e7eb',
                                  borderWidth: `${config.suggestion_border_width || 1}px`,
                                  ...
                                  fontSize: `${config.suggestion_font_size || 12}px`,
                                  padding: `${Math.max(4, Math.min(12, containerWidth * 0.008))}px ${Math.max(8, Math.min(16, containerWidth * 0.015))}px`,
