...
export default function PacksGalleryZone() {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      {packs.map((pack) => (
        <Link href={`/designer-instances/models/train/${pack.slug}`} key={pack.id} className="w-full h-70 bg-black rounded-md overflow-hidden transition-transform duration-300 hover:scale-105">
          <img
